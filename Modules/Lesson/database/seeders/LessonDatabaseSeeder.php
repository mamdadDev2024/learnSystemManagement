...
class LessonDatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        foreach ($courses as $course) {
            ...
            LessonFactory::new()->count($lessonCount)->forCourse($course)->create();

            $this->command->info("✅ {$lessonCount} lessons created for '{$course->title}' course");
        }

        $this->createLessonProgressData();

        $this->command->info('create fake lessons successfully!');
    }

    private function createLessonProgressData(): void
    {
        $users = User::all();
